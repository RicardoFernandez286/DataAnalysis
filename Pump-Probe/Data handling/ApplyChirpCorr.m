function dataStruct = ApplyChirpCorr(dataStruct,chirpSt)
...
Gau1D = @(p,t) p(3).*exp(-log(2).*((t-p(1))./p(2)).^2) + p(4);
...
for j=1:NfitPix
    ...
    [~,idM] = max(abs(Z(:,i)));
    P0(1)   = delays(idM);
    Pfit(j,:) = lsqcurvefit(Gau1D,P0,delays,Z(:,i),LB,UB,options);
...
% Fit dispersion using Cauchy's Equation (truncated on the 3rd term)
% Parameters: 1=a 2=b 3=c;  L = Wavelength in nm
chirpFun = @(p,L) p(1) + 1e7.*p(2)./(L.^2) + 1e9.*p(3)./(L.^4);% + 1e11.*p(4)./(L.^6);
...
plot(ax2,fitWL,1000*(Pfit(:,1)-chirpFun(Cfit,fitWL)),'xk')
yline(ax2,0);
ax.XTickLabel=[];
