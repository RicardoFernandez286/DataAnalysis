function handles = plot_arrow(ax,x1,y1,x2,y2,varargin )
%
...
% =============================================
alpha       = 0.75;   % head length
beta        = 0.75;   % head width
min_length  = 0.5;
min_width   = 0.5;
...
if ~isempty( varargin )
    for c = 1:floor(length(varargin)/2)
        try
            ...
            case 'headheight', alpha = varargin{c*2};
            case 'headwidth',  beta  = varargin{c*2};
            end
        catch
...
head_length = max(line_length*alpha, min_length);
head_width  = max(line_length*beta, min_width);
...
% =============================================
h1          = plot(ax,[x1,x2],[y1,y2],'k' );
h2          = patch(ax,coords(1,:),coords(2,:),[0 0 0]);
