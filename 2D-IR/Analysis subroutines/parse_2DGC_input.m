...
if ~isempty(y0_pos(~isDiagonal)) %If no cross peaks, don't assign these
    ...
    Start_param(Dy_pos(~isDiagonal))= Dy_start(DiagPkYID(~isDiagonal));
end
...
UB(x0_pos(isDiagonal)) = X0_start(isDiagonal)+(15/1000);
UB(y0_pos(isDiagonal)) = X0_start(isDiagonal)+(15/1000);
UB(Dy_pos)             = 80;
UB(Sx_pos(isDiagonal)) = 60;
UB(Sy_pos(isDiagonal)) = 60;
UB(S12_pos(isDiagonal))= 60;

LB(x0_pos(isDiagonal)) = X0_start(isDiagonal)-(15/1000);
LB(y0_pos(isDiagonal)) = X0_start(isDiagonal)-(15/1000);
LB(Dy_pos)             = 5;
